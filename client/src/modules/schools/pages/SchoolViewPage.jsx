  ...
  return (
    <div style={{ backgroundColor: "#ffffff", borderRadius: "16px", border: "1.5px solid #E5E7EB", overflow: "hidden" }}>
      <div style={{ padding: "16px 20px", borderBottom: "1px solid #F3F4F6", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        {count !== undefined && (
          <span style={{ padding: "2px 9px", borderRadius: "20px", fontSize: "11px", fontWeight: "700", backgroundColor: "#F0FDF4", color: TEAL, border: "1px solid #BBF7D0" }}>
            {count}
          </span>
        )}
      </div>
...
const STATUS_LEARNER_LABEL = { active: "Active", inactive: "Inactive", transferred: "Transferred", graduated: "Graduated" };

function Badge({ status, map, labelMap }) {
  const s = map[status] || map.inactive;
  return (
    <span style={{ padding: "2px 8px", borderRadius: "20px", fontSize: "11px", fontWeight: "700", backgroundColor: s.bg, color: s.color, border: `1px solid ${s.border}`, whiteSpace: "nowrap" }}>
...
export default function SchoolViewPage() {
  ...
  if (isLoading) {
    return <div style={{ fontFamily: "Inter, sans-serif", display: "flex", alignItems: "center", justifyContent: "center", minHeight: "200px", color: "#9CA3AF", fontSize: "14px" }}>Loading school…</div>;
  }
  if (isError || !school) {
    return <div style={{ fontFamily: "Inter, sans-serif", padding: "20px 24px", backgroundColor: "#FFF5F5", border: "1px solid #FECACA", borderRadius: "12px", color: "#EF4444", fontSize: "14px" }}>⚠ School not found.</div>;
  }
      ...
      <div style={{ background: "linear-gradient(135deg, #064E3B 0%, #047857 50%, #059669 100%)", borderRadius: "20px", padding: "28px 32px", marginBottom: "20px", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "6px", marginBottom: "16px" }}>
            <button type="button" onClick={() => navigate("/schools")} style={{ background: "none", border: "none", color: "rgba(255,255,255,0.65)", fontSize: "13px", cursor: "pointer", fontFamily: "Inter, sans-serif", padding: 0 }}>Schools</button>
          ...
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "24px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
              <div style={{ width: "60px", height: "60px", borderRadius: "16px", background: "rgba(255,255,255,0.15)", border: "1.5px solid rgba(255,255,255,0.25)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "28px", flexShrink: 0 }}>🏫</div>
              <div>
                <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "4px" }}>
                  <h1 style={{ margin: 0, fontSize: "22px", fontWeight: "900", color: "#ffffff", letterSpacing: "-0.3px" }}>{school.name}</h1>
                  <span style={{ padding: "2px 9px", borderRadius: "20px", fontSize: "11px", fontWeight: "700", backgroundColor: statusStyle.bg, color: statusStyle.color, border: `1px solid ${statusStyle.border}`, textTransform: "capitalize" }}>{school.status}</span>
                </div>
            ...
            <button type="button" onClick={() => navigate(`/schools/${id}/edit`)} style={{ display: "inline-flex", alignItems: "center", gap: "7px", padding: "10px 20px", backgroundColor: "#ffffff", color: TEAL, border: "none", borderRadius: "10px", fontSize: "13px", fontWeight: "700", fontFamily: "Inter, sans-serif", cursor: "pointer", flexShrink: 0, boxShadow: "0 2px 8px rgba(0,0,0,0.15)" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 360px", gap: "16px", alignItems: "start" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
          ...
          <Section title="Assigned Curriculum">
            {curriculum ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "10px", padding: "24px 16px", textAlign: "center" }}>
                ...
                <p style={{ margin: 0, fontSize: "13px", color: "#9CA3AF" }}>No curriculum assigned yet.</p>
                <button type="button" onClick={() => navigate(`/schools/${id}/edit`)} style={{ padding: "8px 18px", backgroundColor: TEAL, color: "#fff", border: "none", borderRadius: "8px", fontSize: "13px", fontWeight: "600", fontFamily: "Inter, sans-serif", cursor: "pointer" }}>Assign Curriculum</button>
              </div>
            )}
          </Section>
          ...
          <Section title="Teachers" count={teachers.length}>
            {teachers.length === 0 ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {teachers.map((t) => (
                  <div key={t.id} onClick={() => navigate(`/teachers/${t.id}/view`)} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 12px", borderRadius: "10px", border: "1px solid #E5E7EB", cursor: "pointer", transition: "background-color 0.12s" }}
                    onMouseEnter={(e) => e.currentTarget.style.backgroundColor = "#F9FAFB"}
                    onMouseLeave={(e) => e.currentTarget.style.backgroundColor = "transparent"}
                  >
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      <div style={{ width: 34, height: 34, borderRadius: "50%", background: "linear-gradient(135deg, #5B21B6, #7C3AED)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, color: "#fff", flexShrink: 0 }}>
                        {(t.firstName?.[0] || "") + (t.lastName?.[0] || "")}
                      </div>
                      <div>
                        <p style={{ margin: 0, fontSize: 13, fontWeight: 600, color: "#111827" }}>{t.firstName} {t.lastName}</p>
                ...
                {teachers.length >= 5 && (
                  <button type="button" onClick={() => navigate(`/teachers?schoolId=${id}`)} style={{ padding: "8px", background: "none", border: "none", color: TEAL, fontSize: 13, fontWeight: 600, fontFamily: "Inter, sans-serif", cursor: "pointer", textAlign: "center" }}>
          ...
          <Section title="Classes" count={classes.length}>
            {classes.length === 0 ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {classes.map((c) => (
                  <div key={c.id} onClick={() => navigate(`/classes/${c.id}/view`)} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 12px", borderRadius: "10px", border: "1px solid #E5E7EB", cursor: "pointer", transition: "background-color 0.12s" }}
                    onMouseEnter={(e) => e.currentTarget.style.backgroundColor = "#F9FAFB"}
                    onMouseLeave={(e) => e.currentTarget.style.backgroundColor = "transparent"}
                  >
                    <div>
                      <p style={{ margin: 0, fontSize: 13, fontWeight: 600, color: "#111827" }}>{c.gradeName}</p>
                      <p style={{ margin: 0, fontSize: 11, color: "#9CA3AF" }}>Academic Year {c.academicYear}{c.capacity ? ` · Capacity ${c.capacity}` : ""}</p>
                    </div>
            ...
            {learners.length === 0 ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {learners.slice(0, 8).map((l) => (
                  <div key={l.id} onClick={() => navigate(`/learners/${l.id}/view`)} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 12px", borderRadius: "10px", border: "1px solid #E5E7EB", cursor: "pointer", transition: "background-color 0.12s" }}
                    onMouseEnter={(e) => e.currentTarget.style.backgroundColor = "#F9FAFB"}
                    onMouseLeave={(e) => e.currentTarget.style.backgroundColor = "transparent"}
                  >
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      <div style={{ width: 34, height: 34, borderRadius: "50%", background: "linear-gradient(135deg, #831843, #BE185D)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, color: "#fff", flexShrink: 0 }}>
                        {(l.firstName?.[0] || "") + (l.lastName?.[0] || "")}
                      </div>
                      <div>
                        <p style={{ margin: 0, fontSize: 13, fontWeight: 600, color: "#111827" }}>{l.firstName} {l.lastName}</p>
                ...
                {learners.length > 8 && (
                  <button type="button" onClick={() => navigate(`/learners?schoolId=${id}`)} style={{ padding: "8px", background: "none", border: "none", color: TEAL, fontSize: 13, fontWeight: 600, fontFamily: "Inter, sans-serif", cursor: "pointer", textAlign: "center" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
          <Section title="School Details">
            <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
              <DetailRow icon={<svg width="14" height="14" viewBox="0 0 24 24" fill="none"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z" stroke="currentColor" strokeWidth="2"/><polyline points="22,6 12,13 2,6" stroke="currentColor" strokeWidth="2"/></svg>} label="Email" value={school.email} />
              <DetailRow icon={<svg width="14" height="14" viewBox="0 0 24 24" fill="none"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12a19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 3.6 1.27h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 8.84a16 16 0 0 0 6 6l.86-.86a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7a2 2 0 0 1 1.72 2.03z" stroke="currentColor" strokeWidth="2"/></svg>} label="Phone" value={school.phone} />
              {address && <DetailRow icon={<svg width="14" height="14" viewBox="0 0 24 24" fill="none"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7z" stroke="currentColor" strokeWidth="2" strokeLinejoin="round"/><circle cx="12" cy="9" r="2.5" stroke="currentColor" strokeWidth="2"/></svg>} label="Address" value={address} />}
              {!school.email && !school.phone && !address && <p style={{ margin: 0, fontSize: "13px", color: "#9CA3AF", textAlign: "center", padding: "12px 0" }}>No contact details yet. <button type="button" onClick={() => navigate(`/schools/${id}/edit`)} style={{ background: "none", border: "none", color: TEAL, fontWeight: "600", cursor: "pointer", fontFamily: "Inter, sans-serif", fontSize: "13px", padding: 0 }}>Add them →</button></p>}
            </div>
          </Section>

          <Section title="Record Info">
            <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
              <DetailRow icon={<svg width="14" height="14" viewBox="0 0 24 24" fill="none"><rect x="3" y="4" width="18" height="18" rx="2" ry="2" stroke="currentColor" strokeWidth="2"/><line x1="16" y1="2" x2="16" y2="6" stroke="currentColor" strokeWidth="2"/><line x1="8" y1="2" x2="8" y2="6" stroke="currentColor" strokeWidth="2"/><line x1="3" y1="10" x2="21" y2="10" stroke="currentColor" strokeWidth="2"/></svg>} label="Created" value={new Date(school.createdAt).toLocaleDateString("en-KE", { day: "numeric", month: "long", year: "numeric" })} />
              <DetailRow icon={<svg width="14" height="14" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="2"/><polyline points="12 6 12 12 16 14" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg>} label="Last Updated" value={new Date(school.updatedAt).toLocaleDateString("en-KE", { day: "numeric", month: "long", year: "numeric" })} />
            </div>
          </Section>
        </div>
      </div>
