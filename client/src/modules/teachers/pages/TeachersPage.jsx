      ...
      <div style={{ background: "linear-gradient(135deg, #1a3550 0%, #25476a 40%, #2e7db5 75%, #38aae1 100%)", borderRadius: "20px", padding: "28px 32px", marginBottom: "20px", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative" }}>
          ...
          <p style={{ margin: 0, fontSize: "13px", color: "rgba(255,255,255,0.72)", lineHeight: "1.5", maxWidth: "560px" }}>
            Pick a school to add and manage its teachers, assign class teachers, and track status.
          </p>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 16 }}>
          {schools.map((school) => {
            ...
            const activeCount = schoolTeachers.filter((t) => t.status === "active").length;
            return (
              <SchoolPickerCard
                key={school.id}
                school={school}
                ...
                countLabel={schoolTeachers.length === 1 ? "teacher" : "teachers"}
                subStat={schoolTeachers.length > 0 ? `${activeCount} active` : null}
                onClick={() => navigate(`/teachers/school/${school.id}`)}
              />
            );
          })}
        </div>
