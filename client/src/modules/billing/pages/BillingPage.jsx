...
import { useCancelInvoice, useCreateInvoice, useIssueInvoice, useInvoicesQuery } from "../hooks/useBilling";
...
export default function BillingPage() {
  ...
  return (
    <div style={{ fontFamily: "Inter, sans-serif", color: "#111827" }}>
      ...
      {!isLearner && showForm && (
        <form onSubmit={submit} style={{ background: "#fff", border: "1px solid #E5E7EB", borderRadius: 14, padding: 20, marginBottom: 18 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(190px, 1fr))", gap: 12 }}>
            {isAdmin && <Field label="Learning hub"><select value={hubId} onChange={(e) => { setHubId(e.target.value); setLearnerId(""); }} style={inputStyle}><option value="">Select a hub</option>{hubs.map((hub) => <option key={hub.id} value={hub.id}>{hub.name}</option>)}</select></Field>}
            ...
            <Field label="Amount (KES)"><input type="number" min="0.01" step="0.01" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="0.00" style={inputStyle} /></Field>
