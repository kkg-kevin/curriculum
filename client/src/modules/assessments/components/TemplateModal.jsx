...
function ItemsEditor({ items, setItems }) {
  ...
  const addItem = () => {
    ...
    if (questionType === "mcq" && options.length < 2) return;
    setItems((prev) => [...prev, {
      question: question.trim(), questionType, options: questionType === "mcq" ? options : [],
      correctAnswer, points: Number(points) || 0,
    }]);
    setQuestion(""); setQuestionType("mcq"); setOptions([]); setOptionInput(""); setCorrectAnswer(""); setPoints(1);
  };
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
          {items.map((item, idx) => (
            <RemovableRow key={idx} onRemove={() => setItems((prev) => prev.filter((_, i) => i !== idx))}>
              ...
              <span style={{ fontSize: "11px", color: "#6B7280" }}>{QUESTION_TYPE_LABELS[item.questionType]} · {item.points} pts{item.options?.length ? ` · ${item.options.length} options` : ""}</span>
            </RemovableRow>
          ))}
        </div>
      )}
      ...
      <div style={{ padding: "12px", backgroundColor: "#F9FAFB", border: "1.5px dashed #E5E7EB", borderRadius: "10px", display: "flex", flexDirection: "column", gap: "10px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 100px", gap: "8px" }}>
          <select style={selectStyle} value={questionType} onChange={(e) => { setQuestionType(e.target.value); setOptions([]); setCorrectAnswer(""); }}>
        ...
        <button
          ...
          disabled={!question.trim() || (questionType === "mcq" && options.length < 2)}
          style={{ alignSelf: "flex-start", padding: "8px 16px", borderRadius: "9px", border: "none", backgroundColor: "#25476a", color: "#fff", fontSize: "12.5px", fontWeight: "700", cursor: "pointer", opacity: (!question.trim() || (questionType === "mcq" && options.length < 2)) ? 0.5 : 1 }}
        >
