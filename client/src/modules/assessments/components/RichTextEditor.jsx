...
export default function RichTextEditor({ value, onChange, minHeight = 120, maxHeight = 260 }) {
  ...
  // tell apart "value changed because we typed" from "value changed externally" (e.g. switching
  // to a different item). Resetting the doc from HTML on every keystroke wipes TipTap's stored
  // marks (the pending bold/underline/etc toggle for the next character), which is why "unbold"
  // previously wouldn't stick.
  ...
  const editor = useEditor({
    ...
    // <style> block, and with identical selectors, the LAST one rendered on the page would win
    // for every editor's height (CSS cascade), not just its own — a real problem here since this
    // component is used with different minHeight/maxHeight per call site. Inline style scopes it
    // correctly per element instead.
    editorProps: {
      attributes: { class: "asm-rte-content", style: `min-height:${minHeight}px;max-height:${maxHeight}px;` },
    },
    onUpdate: ({ editor }) => {
  ...
  return (
    <div style={{ border: "1.5px solid #E5E7EB", borderRadius: "10px", overflow: "hidden", backgroundColor: "#F9FAFB" }}>
      <style>{`
        .asm-rte-content { overflow-y: auto; padding: 10px 12px; outline: none; font-size: 13.5px; color: #374151; line-height: 1.6; }
        ...
        .asm-rte-content video { max-width: 100%; border-radius: 8px; margin: 8px 0; display: block; background: #000; }
        .asm-rte-content strong { font-weight: 700; }
        .asm-rte-content h2 { font-size: 1.25em; font-weight: 800; margin: 4px 0 8px; color: #111827; }
        .asm-rte-content h3 { font-size: 1.1em; font-weight: 700; margin: 4px 0 6px; color: #111827; }
        .asm-rte-content blockquote { margin: 0 0 10px; padding-left: 12px; border-left: 3px solid #a8d5ee; color: #6B7280; font-style: italic; }
        .asm-rte-content code { background: #F3F4F6; padding: 1px 5px; border-radius: 4px; font-size: 0.9em; }
        .asm-rte-content pre { background: #111827; color: #F9FAFB; padding: 10px 12px; border-radius: 8px; overflow-x: auto; margin: 0 0 10px; }
