...
It's a sub-module of Competencies, and it doesn't invent new top-level concepts — it wires together three things that already existed (Learning Areas, Performance Bands, Assessment Types) and adds one new record type on top (the learner's actual placement history).

The three configuration pieces
1. Learning Area — Course Sequence (with stage defaults)
A Learning Area is a named group of related courses (e.g. "Robotics & Automation" → Robotics 1–4). Learning Journey adds a courseSequence: an explicit order for those courses. This is the ladder itself — without an order, the system has courses but no concept of "next." Each sequenced course can also be tagged with one or more Developmental Stages (age-based groupings like "Crib," "Clicker," "Quest") as their default starting point — "a learner in this stage starts at this course." This is a rough, age-based starting guess for a learner nobody has tested yet — not a placement, just a reasonable default. (Previously this lived as a separate per-stage, per-Learning-Area assignment table; it's now just a property of the relevant course-sequence entry, since it's really the same concept — a position in the sequence — not a second one.)

2. Performance Band — Placement Thresholds
...
3. Assessment Type — Learning Area Link
...
Unplaced (computed default). No Journey record exists yet. The system scans the Learning Area's course sequence for an entry whose default stages include the learner's Developmental Stage, and shows that course as their current one. If no entry defaults to that stage, it falls back to the first course in the sequence.
...
Curriculum → Competencies → Learning Journey tab (admin/curriculum-author side): configure the course sequence per area (with stage defaults tagged right on each course), and the placement thresholds. This is where the rules live.
