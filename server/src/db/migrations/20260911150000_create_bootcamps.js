...
//
// Every step below checks the current schema/data before acting: MySQL commits every DDL
// statement immediately (createTable/alterTable are not rolled back by a later failure in the
// same migration, unlike Postgres), so if this migration crashes partway through, a bare retry
// would otherwise error on whatever already succeeded (e.g. "table 'bootcamps' already exists").
// Guarding each step — and making the row-copy idempotent per-curriculum — makes a retry after a
// partial failure a harmless no-op for whatever already ran, instead of requiring manual DB
// surgery.

exports.up = async function up(knex) {
  if (!(await knex.schema.hasTable("bootcamps"))) {
  ...
  if (await knex.schema.hasColumn("curricula", "saleTagline")) {
    ...
    for (const r of rows) {
      ...
      const alreadyCopied = await knex("bootcamps").where({ eventId: r.id }).first();
