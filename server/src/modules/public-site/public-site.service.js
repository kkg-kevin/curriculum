...
// time with the same slugify() bootcamps/projects use. The catalog enforces case-insensitive
// name uniqueness (pathway-template.service.js), so an exact slug clash is unlikely; no
// uniqueSlug()-style disambiguation is added here (see the handoff note). :idOrSlug is resolved
// by findById first, then a findAll() scan for a matching computed slug (getPathway below).
