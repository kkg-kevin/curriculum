...
7. **Extract** `assets.zip` — right-click → Extract. It creates `curriculum.digifunzi.com/assets/` with all the JS/CSS/font files inside. Confirm `assets/index-DjcsDXfi.js` exists after extracting; if the extractor made a nested `assets/assets/`, move it up one level.
...
| File | Purpose |
|---|---|
| `backend-deploy.zip` | Ready-to-upload backend zip — `src/`, `knexfile.js`, `package.json`, `package-lock.json` (code only; no node_modules, no .env, no uploads). Includes every migration through `20260907131200_create_public_diagnostic_attempts.js` (see "This release" above — **read the ⚠️ note before restarting the dev app**). No new dependency this release — `package.json`/`package-lock.json` are unchanged from the previous deploy, so **Run NPM Install** on Restart is a no-op but still safe to click. |
| `assets.zip` | Ready-to-upload frontend assets zip, built with `npm run build` (Dev build — bakes in `https://nodeapp.digifunzi.com`). Zipped as the `assets` **folder**, so it extracts to an `assets/` folder (not loose files). 66 entries (65 asset files + the folder entry). |
| `index.html` | The built frontend entry file (`client/dist/index.html`, Dev build) — upload alongside `assets.zip`, don't extract. Its `<script src>` hash must match the `index-*.js` inside `assets.zip` — both are **`index-DjcsDXfi.js`** in this build; CSS is **`index-CPRP9smp.css`**. |

**Verified before this build was packaged** (Git Bash, from the project root):
```bash
unzip -l Guide/dev/assets.zip | grep -cF '\'          # 0 — no Windows backslash paths
unzip -l Guide/dev/backend-deploy.zip | grep -cF '\'  # 0 — no Windows backslash paths
unzip -l Guide/dev/assets.zip | grep -c '^\s*0.*assets/$'   # 1 — the assets/ folder entry exists
```
All three passed. The zipped `server/src/` is byte-for-byte the local `server/src/` that was
exercised live for this release (multi-tenant isolation + the public pathway/diagnostic work),
and `client/dist/assets/index-DjcsDXfi.js` was grepped to confirm `nodeapp.digifunzi.com` (Dev's
URL) is what's baked in. The `backend-deploy.zip` here is **identical** to `Guide/live/`'s — same
code, only each cPanel Node app's `.env` differs.
